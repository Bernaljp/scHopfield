...
even with no scaffold prior (benchmark M3). This is a recovery test under correct model
specification: it demonstrates identifiability for systems that are in the model's
function class (networks of Hopfield form), and should not be read as evidence that
arbitrary biological networks are faithfully recovered. Mass-action or Michaelis-Menten
circuits that are not natively of Hopfield form are not recovered as faithfully (their
velocity fields are nonetheless well represented; benchmark M9), which we treat as a
scope limitation rather than a general claim. The Hill nonlinearity was essential to
...
This comparison therefore establishes that scHopfield recovers the knockout panel
consistently with known lineage-commitment phenotypes and aligns with CellOracle's
reported native-readout results; it is not a metric-parity ranking, and a strict
identical-endpoint comparison is a planned refinement. Dose-response analyses
...
systems in the model's function class; for mass-action circuits that are not natively of
Hopfield form, the model still represents the dynamics accurately but recovery of a
meaningful effective interaction matrix is limited by identifiability rather than by the
expressiveness of the model: data confined to a low-dimensional trajectory underdetermine
the network, whereas broad coverage of state space (provided on real data by the
neighbour-augmented inference) or a transcription-factor scaffold prior resolves it. Fourth,
the model-derived biological interpretations (the pancreatic Jacobian-stability ordering,
the knockout landscapes, and the dose-response and higher-order predictions) are hypotheses
generated by the inferred dynamical system and require Perturb-seq, targeted knockouts, or
comparable experiments before being treated as established mechanisms. These behaviours are
not specific to a single dataset: across four developmental systems (hematopoiesis, pancreas,
murine neural crest, and human limb) the identifiability and low-rank findings, the
progenitor-to-terminal stability ordering, and the recovery of known lineage regulators all
reproduce (Supplementary S6). Future work could extend the framework to integrate multi-omic,
chromatin-accessibility, spatial, and lineage-tracing data. Taken together, our results establish a unified framework in
