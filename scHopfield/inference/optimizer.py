...
from .._utils.math import HILL_N_MIN
...
class ScaffoldOptimizer(nn.Module):
    ...
    def __init__(
        ...
        # Per-gene data range x_max_i, used by the far-field radial penalty (_radial_loss).
        # ABSOLUTE per-gene floor on the degradation rate. Because the Hill activation saturates,
        # the drive is globally bounded by some D and the field satisfies
        #     r(x) = x^T f(x) <= -gamma_min ||x||^2 + D ||x||,
        # so the system is boundedness-certified BY ARCHITECTURE with absorbing radius D/gamma_min
        # (the "bounded drift plus leak" level of the Theorem-1 hierarchy). No penalty is required;
        # the only way boundedness fails is gamma_i -> 0, which does not unbound the system but
        # sends the absorbing radius to infinity. Flooring gamma therefore fixes the radius directly
        # and applies NO gradient pressure anywhere in the data region.
        self.gamma_min = float(gamma_min)
        if x_max_data is not None:
            xm = torch.as_tensor(x_max_data, dtype=torch.float32, device=device).clamp(min=1e-6)
            self.register_buffer("x_max_data", xm)
        else:
            self.x_max_data = None
        ...
        # Enabled only when a parent W and a positive weight are supplied (fine levels of a hierarchy).
        self.parent_reg_lambda = parent_reg_lambda
        if parent_W is not None and parent_reg_lambda > 0:
            self.register_buffer("parent_W", torch.as_tensor(parent_W, dtype=torch.float32, device=device))
        else:
            self.parent_W = None
    ...
    def forward(self, inputs):
        ...
        I_clamped = self.I
        return self.W(s) + I_clamped - self.effective_gamma() * x

    def effective_gamma(self):
        """The degradation rate actually used by the model: exp(gamma), capped at gamma_max and
        floored at gamma_min.

        The floor is what makes the field boundedness-certified: with a saturating activation the
        drive is bounded by some D, so r(x) = x.f(x) <= -gamma_min ||x||^2 + D ||x||, giving an
        absorbing radius D/gamma_min for any gamma_min > 0. It binds only on genes whose refit rate
        collapsed toward zero and applies no gradient pressure elsewhere.

        Used by forward AND when the fitted gamma is extracted for storage, so training and the
        stored/integrated field are consistent."""
        gamma = torch.exp(self.gamma).clamp(max=self.gamma_max)
        if self.gamma_min > 0:
            gamma = gamma.clamp(min=self.gamma_min)
        return gamma

    def _radial_loss(self, n_samples: int = 256, r_min: float = 1.0, r_max: float = 3.0,
                     alpha: float = 0.0, beta: float = 0.0):
        """Far-field radial (dissipativity) hinge: the ONE term the Theorem-1 hierarchy actually
        requires. With V = ||x||^2/2, V_dot = x^T f(x) =: r(x), and Proposition 1 says that
        r(x) <= -alpha ||x||^2 + beta gives an absorbing ball of radius sqrt(beta/alpha). We penalize
        only violations, on points sampled in the FAR FIELD, so the fit is untouched wherever the
        condition already holds and inside the data region entirely.

        This deliberately omits the other five terms of the six-term menu: `damping` duplicates this
        one, `growth` and `convex` are strictly stronger than boundedness needs, `forcing` is vacuous
        once f(0) ~ 0, and `rotational` is actively harmful, since x^T f_perp(x) = 0 identically
        means circulation cannot affect V_dot and penalizing it only suppresses the oscillations and
        limit cycles the model exists to express (it would damage the repressilator).

        In the far field the Hill activation is saturated (sigma -> 1), so the drive is exactly the
        row sums of W plus I; no Hill parameters are needed to evaluate r out there. Samples are
        drawn in the non-negative orthant (expression cannot be negative) at radii r_min to r_max in
        units of each gene's data range."""
        if self.x_max_data is None:
            return torch.zeros((), device=self.device)
        n = self.W.weight.shape[0]
        drive = self.W.weight.sum(dim=1) + self.I            # saturated drive, sigma -> 1
        gamma = torch.exp(self.gamma).clamp(max=self.gamma_max)
        if self.gamma_min > 0:
            gamma = gamma.clamp(min=self.gamma_min)
        u = torch.randn(n_samples, n, device=self.device).abs()
        u = u / u.norm(dim=1, keepdim=True).clamp(min=1e-12)  # directions in the positive orthant
        rho = r_min + (r_max - r_min) * torch.rand(n_samples, 1, device=self.device)
        x = rho * u * self.x_max_data                         # far-field states, per-gene scaled
        r = (x * (drive[None, :] - gamma[None, :] * x)).sum(dim=1)
        return torch.relu(r + alpha * (x ** 2).sum(dim=1) - beta).mean()
    ...
    def train_model(
        ...
        for epoch in tqdm(range(epochs), desc="Training Epochs", disable=not verbose):
            ...
            for (s_batch, x_batch), target in train_loader:
                ...
                if self.parent_reg_lambda > 0 and self.parent_W is not None:
                    parent_pen = ((self.W.weight - self.parent_W) ** 2).sum()
                    if self.normalize_regularization:
                        parent_pen = parent_pen / batch_size
                    total_loss = total_loss + self.parent_reg_lambda * parent_pen
                if jacobian_lambda > 0 and self._jac is not None:
                    total_loss = total_loss + jacobian_lambda * self._jacobian_loss()
                if boundedness_lambda > 0:
                    total_loss = total_loss + boundedness_lambda * self._radial_loss()
        ...
        # Store learning rate + loss histories on the model for later inspection
        self.lr_history = lr_history
        self.loss_history = loss_history
        self.reconstruction_loss_history = reconstruction_loss_history
...
class HillScaffoldOptimizer(ScaffoldOptimizer):
    ...
    def _model_sigma(self, s, x):
        # the Hill forward ignores the precomputed s and recomputes sigma from x; match that
        # in the boundedness loss so A(x) = W sigma(x) uses the model's own activation
        return self.hill(x)

    def forward(self, inputs):
        _, x = inputs  # precomputed sigma is ignored
        sig = self.hill(x)
        # NOTE: this used to inline the gamma clamp instead of calling effective_gamma(), which
        # silently bypassed any floor applied there. Route through effective_gamma() so the Hill
        # model and the base model use identical degradation, and so the stored gamma matches.
        return self.W(sig) + self.I - self.effective_gamma() * x
