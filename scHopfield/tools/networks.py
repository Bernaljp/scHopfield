...
def compute_network_centrality(
    ...
    for cluster in clusters:
        ...
        # Dominant eigenmode weight (a DIFFERENT quantity from the graph eigenvector
        # centrality above). 'eigenvector_centrality' is the Bonacich centrality of the
        # |W|-weighted directed graph (nonnegative adjacency). This instead is the magnitude
        # of each gene's component in the leading (largest real-part) right eigenvector of the
        # SIGNED interaction matrix W (W x = lambda_max x, Methods Eq. for the dominant
        # eigenmode); it measures participation in the dominant positive-feedback mode, keeping
        # sign information that the graph centrality discards. Both are reported.
        try:
            evals, evecs = np.linalg.eig(np.asarray(W))
            lead = int(np.argmax(evals.real))
            w = np.abs(evecs[:, lead])
            nrm = np.linalg.norm(w)
            result_df['dominant_eigvec_weight'] = (w / nrm) if nrm > 0 else w
        except Exception:
            result_df['dominant_eigvec_weight'] = 0.0
