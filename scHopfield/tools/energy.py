...
def decompose_degradation_energy(
    ...
    # Sigmoid activations (regime-consistent stored layer for bimodal genes) and, for
    # bimodal genes, the raw expression used to assign each cell's Hill regime.
    if x is not None:
        xg = np.asarray(x)
        sig = np.nan_to_num(sigmoid(xg, threshold[None, :], exponent[None, :]))
    else:
    ...
    # Per-gene inverse-Hill integral. For two-component (bimodal) genes each cell uses the
    # (k, n) of its own regime, matching the aggregate _degradation_energy and the activation
    # used in the dynamics, so the gene-wise decomposition sums back to the total.
    if bimodal:
        k2 = adata.var['sigmoid_threshold2'].values[genes]
        n2 = adata.var['sigmoid_exponent2'].values[genes]
        reg = hill_regime(xg, threshold[None, :], k2[None, :])
