...
def build_correlation_projector(
    adata: 'AnnData',
    basis: str = 'umap',
    n_neighbors: int = 200,
    sigma_corr: float = 0.05,
    ...
    """Precompute the embedding neighbour structure once, then project many gene-space
    vectors with the CellOracle-style correlation (transition-probability) scheme.

    ``project_to_embedding(method='correlation')`` rebuilds a dense ``n x n`` transition
    and a dense ``2 x n x n`` direction tensor on EVERY call, even though everything except
    the per-vector correlation depends only on the embedding. For a knockout screen (one
    projection per gene/pair) that is quadratic and dominates the runtime. This builds the
    KNN, the sampled neighbours (identical sampling to the package, same seed), the sampled
    gene-space differences, the sampled embedding unit directions, and the baseline ONCE,
    then returns ``project(vectors) -> (n_cells, 2)`` that only does the cheap per-vector
    correlation, restricted to the sampled neighbours (sparse). The math matches
    ``project_to_embedding(method='correlation', correlation_mode='sampled')``.

    Returns
    -------
    callable
        ``project(vectors)`` where ``vectors`` is ``(n_cells, n_genes_used)`` (gene-space,
        already restricted to ``get_genes_used(adata)``), returning the embedded flow.
    """
    from sklearn.neighbors import NearestNeighbors
    from scipy.sparse import issparse

    embedding = np.asarray(adata.obsm[f'X_{basis}'])[:, :2].astype(np.float64)
    n_cells = embedding.shape[0]
    genes = get_genes_used(adata)
    if spliced_key is None:
        spliced_key = adata.uns.get('scHopfield', {}).get('spliced_key', 'Ms')
    X = adata.layers[spliced_key][:, genes] if spliced_key in adata.layers else adata.X[:, genes]
    if issparse(X):
        X = X.toarray()
    X = np.asarray(X, dtype=np.float64)

    # KNN + per-cell neighbour sampling -- identical to project_to_embedding's sampled path
    np.random.seed(random_seed)
    nn = NearestNeighbors(n_neighbors=n_neighbors + 1, n_jobs=n_jobs).fit(embedding)
    neigh_ixs = nn.kneighbors_graph(mode='connectivity').indices.reshape((-1, n_neighbors + 1))
    p = np.linspace(sampling_probs[0], sampling_probs[1], neigh_ixs.shape[1]); p = p / p.sum()
    n_sampled = int(sampled_fraction * (n_neighbors + 1))
    sampling_ixs = np.stack(
        [np.random.choice(neigh_ixs.shape[1], size=n_sampled, replace=False, p=p)
         for _ in range(n_cells)], axis=0)
    neigh = neigh_ixs[np.arange(n_cells)[:, None], sampling_ixs]        # (n, n_sampled)
    self_mask = neigh == np.arange(n_cells)[:, None]

    # gene-space differences to sampled neighbours (centred, for Pearson)
    Dg = X[neigh] - X[:, None, :]                                       # (n, n_sampled, g)
    Dg_c = Dg - Dg.mean(axis=2, keepdims=True)
    Dg_ss = np.sqrt(np.sum(Dg_c ** 2, axis=2))                          # (n, n_sampled)

    # embedding unit directions to sampled neighbours + vector-independent baseline
    Ud = embedding[neigh] - embedding[:, None, :]                       # (n, n_sampled, 2)
    Ud = Ud / (np.linalg.norm(Ud, axis=2, keepdims=True) + 1e-10)
    Ud[self_mask] = 0.0
    baseline = Ud.sum(axis=1) / (n_sampled + 1e-10)                     # (n, 2)

    def project(vectors: np.ndarray) -> np.ndarray:
        v = np.asarray(vectors, dtype=np.float64)
        v_c = v - v.mean(axis=1, keepdims=True)
        v_ss = np.sqrt(np.sum(v_c ** 2, axis=1))                        # (n,)
        num = np.einsum('ig,isg->is', v_c, Dg_c)                        # (n, n_sampled)
        corr = num / (v_ss[:, None] * Dg_ss + 1e-10)
        corr[self_mask] = 0.0
        tp = np.exp(corr / sigma_corr)
        tp = tp / (tp.sum(axis=1, keepdims=True) + 1e-10)
        return np.einsum('is,isd->id', tp, Ud) - baseline               # (n, 2)
