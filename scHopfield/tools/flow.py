...
def calculate_flow(
    ...
    method : str, optional (default: 'hopfield')
        PROJECTION method: how the (gene-space) perturbation is mapped onto the 2D
        ...
        - 'correlation' ('celloracle'): project the raw displacement delta_X onto the embedding
          using the neighbor-correlation transition-probability scheme (the scVelo/CellOracle-
          style projection). This projects the displacement; it does not re-simulate anything.
          The 'celloracle' alias is kept for backward compatibility but the name is misleading
          (it refers to the projection style, not the CellOracle perturbation tool).
    ...
    """
    # Normalize projection-method aliases. The old names ('hopfield'/'celloracle') conflate
    # the projection with the CellOracle simulation tool; 'model_velocity'/'correlation' say
    # what they actually do. Both spellings are accepted.
    _PROJ = {'hopfield': 'model_velocity', 'model': 'model_velocity', 'model_velocity': 'model_velocity',
             'celloracle': 'correlation', 'correlation': 'correlation', 'transition': 'correlation'}
    proj = _PROJ.get(method, method)
    if proj == 'model_velocity':
        ...
        raise ValueError(f"Unknown projection method: {method!r}. Use 'model_velocity' "
                         f"('hopfield') or 'correlation' ('celloracle').")
