...
def run_ko_screen(
    ...
    # Build the correlation (transition-probability) projector ONCE; each KO's lineage bias
    # reuses it (matches the reference velocity + flow visualizations, and is ~150x faster
    # than reprojecting per gene).
    from ..tools.embedding import build_correlation_projector
    projector = build_correlation_projector(adata, basis=basis)
    ...
    for gene in genes:
        ...
        bias_dict[gene]    = compute_perturbation_flow_bias(
            ...
            basis, wt_flow_key,
            cluster_key=cluster_key, projector=projector,
        )
...
def run_pairwise_ko_screen(
    ...
    # Correlation (transition-probability) projector, built once and reused per pair.
    from ..tools.embedding import build_correlation_projector
    projector = build_correlation_projector(adata, basis=basis)
...
def run_dose_response(
    ...
    # Correlation (transition-probability) projector, built once and reused per dose level.
    from ..tools.embedding import build_correlation_projector
    projector = build_correlation_projector(adata, basis=basis)
    ...
    for level in iter_levels:
        ...
        calculate_flow(
            adata_t, source='delta', basis=basis, method='correlation',
            cluster_key=cluster_key,
