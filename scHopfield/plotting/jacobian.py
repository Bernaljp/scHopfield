...
def plot_jacobian_element_grid(
    ...
    **scatter_kwargs
    ...
    for idx, (gene_i, gene_j) in enumerate(gene_pairs):
        ...
        # A Jacobian element is identically zero whenever the regulator is not a TF, since
        # only_TFs hard-zeros those columns of W. TwoSlopeNorm then gets vmin=vcenter=vmax=0 and
        # raises "must be in ascending order", which killed the whole panel. Fall back to a small
        # symmetric range so the (legitimately flat) element still renders.
