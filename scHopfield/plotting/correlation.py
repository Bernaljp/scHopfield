...
def plot_correlations_grid(
    ...
    **kwargs
    ...
    n = len(cell_types)
    # scale the grid with the number of cell types so panels do not shrink to nothing on
    # datasets with many clusters (e.g. murine neural crest, human limb)
    figsize = kwargs.get('figsize', (max(15, 2.1 * n), max(15, 2.1 * n)))
    ...
    for i in range(n):
        for j in range(i, n):
            if i == j:
                ...
                # Add the cell-type name in the middle. Wrap long annotations and shrink the
                # font so they do not overflow the small diagonal panels (long names, e.g. on
                # murine neural crest / human limb, were clipping and squeezing the grid).
                import textwrap
                label = str(cell_types[i])
                wrapped = textwrap.fill(label.replace('-', ' '), width=12)
                longest = max((len(w) for w in wrapped.split('\n')), default=len(label))
                fs = 18 if longest <= 10 else (14 if longest <= 14 else 11)
                axs[i, j].text(
                    0.5, 0.5, wrapped,
                    ha='center', va='center',
