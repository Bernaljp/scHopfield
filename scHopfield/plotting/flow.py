...
Computation functions have been moved to:
...
# Import from tools for use in plotting
from ..tools.flow import calculate_flow, calculate_grid_flow, calculate_inner_product
from ..tools.velocity import compute_velocity, compute_velocity_delta
...
# These maintain backward compatibility with old function names
# =============================================================================

def compute_hopfield_velocity(*args, **kwargs):
    """DEPRECATED: Use scHopfield.tl.compute_velocity instead."""
    warnings.warn(
        "compute_hopfield_velocity is deprecated. "
        "Use scHopfield.tl.compute_velocity instead.",
        DeprecationWarning, stacklevel=2
    )
    return compute_velocity(*args, **kwargs)


def compute_hopfield_velocity_delta(*args, **kwargs):
    """DEPRECATED: Use scHopfield.tl.compute_velocity_delta instead."""
    warnings.warn(
        "compute_hopfield_velocity_delta is deprecated. "
        "Use scHopfield.tl.compute_velocity_delta instead.",
        DeprecationWarning, stacklevel=2
    )
    return compute_velocity_delta(*args, **kwargs)


def compute_hopfield_velocity_at_state(adata, X, cluster_key='cell_type', use_cluster_specific_GRN=True):
    """DEPRECATED: Use scHopfield.tl.compute_velocity instead."""
    warnings.warn(
        "compute_hopfield_velocity_at_state is deprecated. "
        "Use scHopfield.tl.compute_velocity(adata, X=X) instead.",
        DeprecationWarning, stacklevel=2
    )
    return compute_velocity(adata, X=X, cluster_key=cluster_key, use_cluster_specific=use_cluster_specific_GRN)


def project_velocity_to_embedding(*args, **kwargs):
    """DEPRECATED: Use scHopfield.tl.project_to_embedding instead."""
    warnings.warn(
        "project_velocity_to_embedding is deprecated. "
        "Use scHopfield.tl.project_to_embedding instead.",
        DeprecationWarning, stacklevel=2
    )
    return project_to_embedding(*args, **kwargs)


def calculate_perturbation_flow(
    adata, basis='umap', method='celloracle', n_neighbors=200,
    sigma_corr=0.05, correlation_mode='sampled', sampled_fraction=0.3,
    sampling_probs=(0.5, 0.1), cluster_key='cell_type',
    use_cluster_specific_GRN=True, velocity_type='delta',
    random_seed=42, n_jobs=4, verbose=True
):
    """DEPRECATED: Use scHopfield.tl.calculate_flow instead."""
    warnings.warn(
        "calculate_perturbation_flow is deprecated. "
        "Use scHopfield.tl.calculate_flow instead.",
        DeprecationWarning, stacklevel=2
    )
    return calculate_flow(
        adata, source=velocity_type, basis=basis, method=method,
        cluster_key=cluster_key, use_cluster_specific=use_cluster_specific_GRN,
...
def calculate_perturbation_flow_hopfield(
    adata, basis='umap', cluster_key='cell_type',
    use_cluster_specific_GRN=True, velocity_type='delta',
    n_neighbors=30, n_jobs=4, verbose=True
):
    """DEPRECATED: Use scHopfield.tl.calculate_flow with method='hopfield' instead."""
    warnings.warn(
        "calculate_perturbation_flow_hopfield is deprecated. "
        "Use scHopfield.tl.calculate_flow(method='hopfield') instead.",
        DeprecationWarning, stacklevel=2
    )
    return calculate_flow(
        adata, source=velocity_type, basis=basis, method='hopfield',
        cluster_key=cluster_key, use_cluster_specific=use_cluster_specific_GRN,
        n_neighbors=n_neighbors, n_jobs=n_jobs, verbose=verbose
    )
...
def calculate_perturbed_velocity_flow(
    adata, basis='umap', cluster_key='cell_type',
    use_cluster_specific_GRN=True, n_neighbors=30, n_jobs=4, verbose=True
):
    """DEPRECATED: Use scHopfield.tl.calculate_flow(source='perturbed') instead."""
    warnings.warn(
        "calculate_perturbed_velocity_flow is deprecated. "
        "Use scHopfield.tl.calculate_flow(source='perturbed') instead.",
        DeprecationWarning, stacklevel=2
    )
    return calculate_flow(
        adata, source='perturbed', basis=basis, method='hopfield',
        cluster_key=cluster_key, use_cluster_specific=use_cluster_specific_GRN,
        n_neighbors=n_neighbors, n_jobs=n_jobs, verbose=verbose
    )
...
def calculate_original_velocity_flow(
    adata, basis='umap', cluster_key='cell_type',
    use_cluster_specific_GRN=True, spliced_key='Ms',
    n_neighbors=30, n_jobs=4, verbose=True
):
    """DEPRECATED: Use scHopfield.tl.calculate_flow(source='original') instead."""
    warnings.warn(
        "calculate_original_velocity_flow is deprecated. "
        "Use scHopfield.tl.calculate_flow(source='original') instead.",
        DeprecationWarning, stacklevel=2
    )
    return calculate_flow(
        adata, source='original', basis=basis, method='hopfield',
        cluster_key=cluster_key, use_cluster_specific=use_cluster_specific_GRN,
        n_neighbors=n_neighbors, n_jobs=n_jobs, verbose=verbose,
        original_key=spliced_key
    )


def calculate_grid_flow_old(
    adata, basis='umap', n_grid=40, smooth=0.5, min_mass=1.0, flow_key=None
):
    """DEPRECATED: Use scHopfield.tl.calculate_grid_flow with method='gaussian' instead."""
    warnings.warn(
        "calculate_grid_flow from plotting.flow is deprecated. "
        "Use scHopfield.tl.calculate_grid_flow instead.",
        DeprecationWarning, stacklevel=2
    )
    if flow_key is None:
        flow_key = f'perturbation_flow_{basis}'
    return calculate_grid_flow(
        adata, flow_key=flow_key, basis=basis, n_grid=n_grid,
        method='gaussian', smooth=smooth, min_mass=min_mass
    )


def calculate_grid_flow_knn(
    adata, basis='umap', n_grid=40, n_neighbors=200,
    min_mass=1.0, flow_key=None, n_jobs=4
):
    """DEPRECATED: Use scHopfield.tl.calculate_grid_flow with method='knn' instead."""
    warnings.warn(
        "calculate_grid_flow_knn is deprecated. "
        "Use scHopfield.tl.calculate_grid_flow(method='knn') instead.",
        DeprecationWarning, stacklevel=2
    )
    if flow_key is None:
        flow_key = f'perturbation_flow_{basis}'
    return calculate_grid_flow(
        adata, flow_key=flow_key, basis=basis, n_grid=n_grid,
        method='knn', n_neighbors=n_neighbors, min_mass=min_mass, n_jobs=n_jobs
    )


def calculate_inner_product_old(
    adata, basis='umap', velocity_key=None, perturbation_flow_key=None
):
    """DEPRECATED: Use scHopfield.tl.calculate_inner_product instead."""
    warnings.warn(
        "calculate_inner_product from plotting.flow is deprecated. "
        "Use scHopfield.tl.calculate_inner_product instead.",
        DeprecationWarning, stacklevel=2
    )
    if velocity_key is None:
        velocity_key = f'velocity_{basis}'
    ...
    return calculate_inner_product(
        adata, velocity_key, perturbation_flow_key, normalize=True, store=True
    )


def calculate_ode_trajectory_flow(*args, **kwargs):
    """DEPRECATED: Use scHopfield.dyn.calculate_trajectory_flow instead."""
    warnings.warn(
        "calculate_ode_trajectory_flow is deprecated. "
        "Use scHopfield.dyn.calculate_trajectory_flow instead.",
        DeprecationWarning, stacklevel=2
    )
    from ..dynamics.simulation import calculate_trajectory_flow as ctf
    return ctf(*args, **kwargs)


def calculate_ode_trajectory_inner_product(
    adata, basis='umap', velocity_key=None, flow_key=None
):
    """DEPRECATED: Use scHopfield.tl.calculate_inner_product instead."""
    warnings.warn(
        "calculate_ode_trajectory_inner_product is deprecated. "
        "Use scHopfield.tl.calculate_inner_product instead.",
        DeprecationWarning, stacklevel=2
    )
    if velocity_key is None:
    ...
    if flow_key is None:
        flow_key = f'ode_perturbation_flow_{basis}'
    return calculate_inner_product(
        adata, velocity_key, flow_key, normalize=True, store=True,
        store_key='ode_perturbation_inner_product'
    )


# Plotting function aliases (these remain but call new functions)
def plot_perturbation_flow(
    adata, basis='umap', flow_key=None, ax=None, scale=1.0,
    color='#EC7063', alpha=0.8, show_background=True,
    cluster_key=None, colors=None, s=10, figsize=(8, 8),
    title='Perturbation Flow', **quiver_kwargs
):
    """Plot perturbation-induced flow (calls plot_flow)."""
    if flow_key is None:
        flow_key = f'perturbation_flow_{basis}'
    return plot_flow(
        adata, flow_key=flow_key, basis=basis, ax=ax, scale=scale,
        color=color, alpha=alpha, show_background=show_background,
        cluster_key=cluster_key, colors=colors, s=s, figsize=figsize,
        title=title, **quiver_kwargs
    )


def plot_perturbed_velocity_flow(
    adata, basis='umap', ax=None, scale=1.0, color='#27AE60',
    alpha=0.8, show_background=True, cluster_key=None,
    colors=None, s=10, figsize=(8, 8),
    title='Velocity at Perturbed State', **quiver_kwargs
):
    """Plot velocity at perturbed state (calls plot_flow)."""
...
def plot_flow_on_grid(
    adata, flow_key=None, basis='umap', n_grid=40, n_neighbors=200,
    min_mass=1.0, recalculate=False, n_jobs=4, ax=None, scale=1.0,
    arrow_color='black', alpha=0.8, show_background=True,
    cluster_key=None, colors=None, s=10, figsize=(8, 8),
    title=None, **quiver_kwargs
):
    """Plot flow on grid (calls plot_flow with on_grid=True)."""
    return plot_flow(
        adata, flow_key=flow_key, basis=basis, ax=ax, on_grid=True,
        scale=scale, color=arrow_color, alpha=alpha,
        show_background=show_background, cluster_key=cluster_key,
        colors=colors, s=s, figsize=figsize, title=title,
        n_grid=n_grid, n_neighbors=n_neighbors, min_mass=min_mass,
        recalculate=recalculate, n_jobs=n_jobs, **quiver_kwargs
    )


def plot_simulation_flow_on_grid(
    adata, basis='umap', n_grid=40, n_neighbors=200, min_mass=1.0,
    scale=1.0, ax=None, show_background=True, cluster_key=None,
    colors=None, arrow_color='black', s=10, alpha=0.8,
    figsize=(8, 8), title=None, flow_key=None, recalculate=False,
    n_jobs=4, **quiver_kwargs
):
    """Plot simulation flow on grid (calls plot_flow with on_grid=True)."""
    return plot_flow(
        adata, flow_key=flow_key, basis=basis, ax=ax, on_grid=True,
        scale=scale, color=arrow_color, alpha=alpha,
        show_background=show_background, cluster_key=cluster_key,
...
def plot_inner_product_on_embedding(
    adata, basis='umap', ax=None, vmin=-1, vmax=1, cmap='RdBu_r',
    s=15, figsize=(8, 8), title='Inner Product\n(Perturbation vs Reference)',
    show_colorbar=True
):
    """Plot inner product on embedding (calls plot_inner_product)."""
    return plot_inner_product(
        adata, basis=basis, by_cluster=False, ax=ax, vmin=vmin, vmax=vmax,
        cmap=cmap, s=s, figsize=figsize, title=title, show_colorbar=show_colorbar
    )


def plot_inner_product_by_cluster(
    adata, cluster_key='cell_type', ax=None, order=None,
    colors=None, figsize=(10, 5), title='Inner Product by Cluster'
):
    """Plot inner product by cluster (calls plot_inner_product)."""
    return plot_inner_product(
        adata, by_cluster=True, cluster_key=cluster_key, ax=ax,
        figsize=figsize, title=title, order=order, colors=colors
    )
...
# Visualization alias
def visualize_velocity_comparison(*args, **kwargs):
    """DEPRECATED: Use visualize_flow_comparison instead."""
    warnings.warn(
        "visualize_velocity_comparison is deprecated. "
        "Use visualize_flow_comparison instead.",
        DeprecationWarning, stacklevel=2
    )
    return visualize_flow_comparison(*args, **kwargs)
