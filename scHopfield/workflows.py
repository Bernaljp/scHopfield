...
def run_pipeline(
    ...
    *,
    ...
    base_grn=None,
    scaffold_kwargs: Optional[Dict] = None,
    ...
    scaffold
        Prior-knowledge mask passed to ``fit_interactions`` as ``w_scaffold``,
        already in regulator-by-target orientation. Pass ``base_grn`` instead to
        have it built here.
    base_grn
        Wide base-GRN table, as :func:`~scHopfield.inference.build_scaffold`
        expects. When given and ``scaffold`` is ``None``, the scaffold is built
        from it after gene selection, so it matches the genes actually modelled.
        This is the scaffold-first path, and it is the published method.

        With neither ``scaffold`` nor ``base_grn``, an unconstrained
        (pseudoinverse) GRN is fit. That is a different method: the ablations in
        the paper find the scaffold essential, so treat the unconstrained fit as
        a baseline rather than as a result.
    scaffold_kwargs
        Extra kwargs for ``build_scaffold`` when building from ``base_grn``.
    fit_kwargs
    ...
    # --- scaffold ---
    # Built after gene selection so its gene universe is the one being modelled.
    if scaffold is None and base_grn is not None:
        sc_df = inf.build_scaffold(a, base_grn, **dict(scaffold_kwargs or {}))
        scaffold = np.asarray(sc_df.values).T
        _mark(f"build_scaffold ({int((scaffold != 0).sum())} edges)")
