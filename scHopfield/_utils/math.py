...
def fit_sigmoid(g, min_th=0.05, n_min=HILL_N_MIN, n_max=8.0, refine=True):
    """
...
def fit_sigmoid_bimodal(g, min_th=0.05, n_min=HILL_N_MIN, n_max=20.0, margin=0.6,
                        min_single_mse=1e-4):
...
def int_sig_act_inv(x, s, n, verbose=False):
    ...
    z = -(n_safe / (n_safe - 1)) * s * hyper(-1 / n_safe, (n_safe - 1) / n_safe, (2 * n_safe - 1) / n_safe, 1)
    z = z[None, :]
    ns = n_safe[None, :]
    ss = s[None, :]
    z1 = -ns * ss * (1 - x) ** ((ns - 1) / ns) * hyper(-1 / ns, (ns - 1) / ns, (2 * ns - 1) / ns, 1 - x) / (ns - 1)
    out = z1 - z

    if near1.any():
        mm = s[None, :] * (-x - np.log(np.clip(1.0 - x, 1e-12, None)))
        out = np.where(near1[None, :], mm, out)
