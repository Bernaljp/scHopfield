...
without re-fitting. Outputs live under ``reports/<dataset>/``.
"""
import os
import warnings
...
def prepare_and_fit(name, device="cuda", force=False, mode=None, tag="", bimodal=None):
    ...
    # drop unwanted clusters (e.g. schwann's unassigned 'none') before anything else
    excl = cfg.get("exclude_clusters")
    if excl:
        keep_mask = ~adata.obs[ck].astype(str).isin([str(e) for e in excl]).values
        if keep_mask.sum() < adata.n_obs:
            print(f"[{name}] excluding clusters {excl}: {adata.n_obs} -> {int(keep_mask.sum())} cells", flush=True)
    ...
    # drop genes whose velocity is not finite everywhere: dynamo's velocity layer
    # (velocity_alpha_minus_gamma_s) carries NaN for genes it could not estimate, and
    # zeroing those injects fake static dynamics that still destabilize the fit (NaN loss
    # from epoch 1). Remove them and keep only the finite-velocity genes. No-op for datasets
    # whose velocities are already finite (dynamo_hematopoiesis: 1956 -> 1728).
    if "velocity_S" in adata.layers:
        V = adata.layers["velocity_S"]
        V = np.asarray(V.todense()) if hasattr(V, "todense") else np.asarray(V, dtype=float)
        finite_gene = np.isfinite(V).all(axis=0)
        if not finite_gene.all():
            print(f"[{name}] dropping {int((~finite_gene).sum())} genes with non-finite "
                  f"velocity ({int(finite_gene.sum())} finite genes left)", flush=True)
            adata = adata[:, finite_gene].copy()

    # sanitize any residual NaN/inf in expression (Ms) *before* gene selection: dynamo-
    # processed and raw datasets can carry NaN/inf that would corrupt the top-velocity
    # ranking and produce NaN weights downstream.
    ...
    # Fix the arbitrary pseudotime timescale (see PSEUDOTIME_RATE_TARGET in config.py). Applied
    # AFTER gene selection so the target is hit on the genes actually fitted; the selection itself
    # is unaffected because it ranks by velocity magnitude and this scaling is uniform.
    if vmode == "pseudotime":
        _V = np.asarray(sub.layers["velocity_S"], dtype=float)
        _X = np.asarray(sub.layers["Ms"], dtype=float)
        # per-gene turnover rate, then the MEDIAN gene (robust to a few fast genes)
        _rg = np.sqrt(np.nanmean(_V ** 2, axis=0)) / np.maximum(
            np.sqrt(np.nanmean(_X ** 2, axis=0)), 1e-12)
        _rate = float(np.nanmedian(_rg))
        if np.isfinite(_rate) and _rate > 0:
            _f = PSEUDOTIME_RATE_TARGET / _rate
            sub.layers["velocity_S"] = _V * _f
            print(f"[{name}] pseudotime velocity rescaled by {_f:.4g} "
                  f"(median-gene rate {_rate:.3f} -> {PSEUDOTIME_RATE_TARGET})", flush=True)
...
# --------------------------------------------------------------------------- #
def present_clusters(adata, ck, min_cells=5):
