...
def _stream(ax, adata, ck, colors, basis, velocity, title=None, color_key=None, emb_key=None):
    ...
    If ``emb_key`` names an embedded velocity already in ``adata.obsm`` (e.g. dynamo's
    precomputed ``velocity_umap``), that projection is used directly and NOT recomputed."""
    ...
    if emb_key and emb_key in a.obsm:
        # use the dataset's own embedded velocity; draw a grid quiver (no reprojection)
        tmp = "_emb_input_tmp"
        a.obsm[tmp] = np.asarray(a.obsm[emb_key])[:, :2].copy()
        _flow_grid(ax, a, tmp, basis, ck, colors)
        del a.obsm[tmp]
...
# One panel can be drawn by dynamo instead of scVelo, which needs its own interpreter
# and a helper script that belongs to the analysis pipeline rather than to this
# repository. Both are optional: when either is unset or missing, the caller falls
# back to a scVelo stream plot. See reproducibility/paths.py.
DYN_PY = paths.DYNAMO_PYTHON
DYN_STREAMLINE = paths.DYN_STREAMLINE


def _dyn_stream(name, fname, adata, ck, colors, basis, velocity, title="", emb_key=None):
    ...
    If ``emb_key`` names an embedded velocity already in ``adata.obsm`` (e.g. dynamo's
    precomputed ``velocity_umap``), that projection is passed through directly and the
    scVelo reprojection is skipped."""
    import subprocess
    ...
    if not FORCE_FIGS and os.path.exists(path):
        return rel
    if not DYN_PY or not os.path.exists(DYN_PY):
        return None
    if not DYN_STREAMLINE or not os.path.exists(DYN_STREAMLINE):
        return None
    try:
        if emb_key and emb_key in adata.obsm:
            emb_vel = np.asarray(adata.obsm[emb_key])[:, :2]
        else:
            ...
            emb_vel = np.asarray(adata.obsm[f"velocity_{basis}"])[:, :2]
        ...
        m.write(hp)
        r = subprocess.run([DYN_PY, DYN_STREAMLINE, "--input", hp,
...
def _flow_panel(report, name, fname, adata, ck, colors, basis, velocity, title, caption,
    ...
    When ``emb_key`` is given and present in ``adata.obsm``, the dataset's own embedded
    velocity is used directly instead of reprojecting the per-cell velocity."""
    rel = _dyn_stream(name, fname, adata, ck, colors, basis, velocity, title, emb_key=emb_key)
    if rel:
        report.img(rel, caption)
        return
    def _fig():
        fig, ax = plt.subplots(figsize=(7, 6))
        _stream(ax, adata, ck, colors, basis, velocity, title=title, emb_key=emb_key)
        return fig
    _try(report, name, fname, _fig, caption)


def _flow_grid(ax, adata, flow_key, basis, ck, colors, color="black", size=1.0,
               n_grid=25, min_mass=25):
    """Grid-averaged flow via the package plot_flow (on_grid), drawn so the arrows are
    ALWAYS legible regardless of the field's absolute magnitude.

    Different flow fields live on very different velocity scales -- the reference (WT)
    velocity, a KO displacement, and an OE displacement can differ by orders of magnitude,
    and the same is true across datasets. A single fixed quiver ``scale`` therefore makes
    arrows either vanish or swamp the whole embedding. Instead we grid the (outlier-clipped)
    field ourselves, then set the quiver scale in DATA coordinates (``scale_units='xy'``) so
    ...
    adata.obsm[flow_key] = F
    # grid the clipped field and cache it so plot_flow reuses exactly this grid
    grid = calculate_grid_flow(adata, flow_key=flow_key, basis=basis, n_grid=n_grid,
                               min_mass=min_mass, n_jobs=4)
    adata.uns[f"grid_flow_{flow_key}"] = grid
    coords = np.asarray(grid["grid_coords"]); valid = ~np.asarray(grid["mass_filter"])
    gmag = np.linalg.norm(np.asarray(grid["grid_flow"])[valid], axis=1) if valid.any() else np.array([1.0])
    # target arrow length = size * one grid cell; scale_units='xy' -> len = |vec| / qscale
    span = float(np.mean(coords.max(0) - coords.min(0)))
    cell = span / max(n_grid, 1)
    ...
    sch.pl.plot_flow(adata, flow_key=flow_key, basis=basis, ax=ax, on_grid=True,
                     n_grid=n_grid, min_mass=min_mass, scale=qscale, color=color,
                     cluster_key=ck, colors=colors, recalculate=False,
...
def section_A(adata, name, ck, report, cfg):
    ...
    have_emb = bool(vek) and (vek in adata.obsm)
    proj_note = (f"the input velocity uses the dataset's own precomputed `{vek}` embedding "
                 f"(no reprojection); the fitted velocity is projected to the embedding with "
                 f"scVelo") if have_emb else (
                 "the fitted velocity is projected to the embedding with scVelo")
    report.sub("1.2 Dynamics on the embedding",
               f"Velocity fields drawn with dynamo's streamline plotter ({proj_note}, then "
               "rendered by dynamo). The fitted Hopfield velocity should reproduce the input -- "
               "progenitors flow to the terminal states.")
    vlab = "pseudotime" if cfg["velocity_mode"] == "pseudotime" else "RNA"
    in_cap = (f"Input {vlab} velocity field, using the dataset's precomputed `{vek}` embedding "
              "(not reprojected).") if have_emb else f"Input {vlab} velocity field (dynamo streamlines)."
...
def section_C(adata, name, ck, report, cfg):
    ...
    # The centrality module now reports TWO distinct eigenvector-based scores: the graph
    # (Bonacich) 'eigenvector_centrality' on the |W| digraph, and 'dominant_eigvec_weight',
    # the magnitude of each gene's component in the leading eigenvector of the SIGNED W
    # (participation in the dominant regulatory mode). Recompute centrality if the cached
    # AnnData predates the addition of 'dominant_eigvec_weight'.
    if f"dominant_eigvec_weight_{order[0]}" not in adata.var.columns:
        try:
            sch.tl.compute_network_centrality(adata, cluster_key=ck)
        except Exception as exc:
            print(f"  [C3] recompute centrality failed: {exc}", flush=True)

    report.sub("3.3 Centrality-measure comparison",
               "Graph eigenvector centrality (Bonacich influence on the $|W|$ network) vs "
               "betweenness centrality (bottleneck / information-flow role). These capture "
               "complementary notions of importance: an influential hub is not necessarily a "
               "bottleneck. Colored by cell type.")
    _try(report, name, "C3_centrality_scatter.png",
         ...
         "Graph (Bonacich) eigenvector vs betweenness centrality per cell type.")

    report.sub("3.3b Dominant eigenmode participation",
               "The other eigenvector-based score: `dominant_eigvec_weight`, the magnitude of "
               "each gene's component in the leading eigenvector of the **signed** interaction "
               "matrix $W$ (participation in the dominant regulatory / positive-feedback mode). "
               "This is a different quantity from the graph eigenvector centrality above (it "
               "keeps the sign structure of $W$); genes off the diagonal here are the drivers of "
               "the dominant dynamical mode rather than topological hubs.")
    if f"dominant_eigvec_weight_{order[0]}" in adata.var.columns:
        _try(report, name, "C3b_dominant_eigvec.png",
             lambda: sch.pl.plot_centrality_scatter(adata, x_metric="dominant_eigvec_weight",
                                                    y_metric="eigenvector_centrality",
                                                    cluster_key=ck, order=order, colors=colors),
             "Dominant eigenmode participation (signed-W leading eigenvector) vs graph "
             "eigenvector centrality; the two differ, showing they measure different things.")
...
def section_D(adata, name, ck, report, cfg):
    ...
    if pairs:
        try:
            ...
            # plot_jacobian_element_grid hard-codes X_umap. TEMPORARILY alias X_umap to the
            # report's basis so the element-wise dynamics use the SAME embedding as the rest
            # of the report. Previously this only aliased when X_umap was ABSENT, so datasets
            # that have BOTH a primary embedding (basis_of, e.g. draw_graph_fa) AND X_umap
            # (e.g. human_limb) drew D.4 on a different projection than every other panel.
            _saved_umap = adata.obsm["X_umap"].copy() if "X_umap" in adata.obsm else None
            if basis != "umap":
                adata.obsm["X_umap"] = np.asarray(adata.obsm[f"X_{basis}"])
            def _je():
            ...
            try:
                _try(report, name, "D4_jac_elements.png", _je,
                     f"Jacobian element (regulator->target sensitivity) dynamics for {len(pairs)} "
                     f"hub pairs (on the {basis} embedding, same as the rest of the report).")
            finally:
                if basis != "umap":
                    if _saved_umap is not None:
                        adata.obsm["X_umap"] = _saved_umap
                    else:
...
# The perturbation displacement delta_X is PROJECTED onto the embedding with the
# neighbor-correlation transition-probability scheme (calculate_flow method="correlation",
# formerly "celloracle"): this is the projection, distinct from how the perturbation was
# simulated. Both simulators (discrete propagation and the ODE) feed the same projection.
def _perturb(adata, cond, ck, basis):
    """Discrete CellOracle-style perturbation: iterative GRN signal propagation (Methods 8.1)."""
...
def _perturb_ode(adata, cond, ck, basis):
    """ODE version: integrate the fitted Hopfield dynamics dx/dt = W.sigma(x) - gamma.x + I
    under the perturbation (Methods 8.2), with the perturbed genes held fixed. Stores the same
    delta_X layer as the discrete propagation, so the two are directly comparable."""
    pert = sch.dyn.simulate_shift_ode(adata, perturb_condition=cond, cluster_key=ck,
                                      n_steps=100, method="euler")
    sch.tl.calculate_flow(pert, source="delta", basis=basis, method="correlation",
                          cluster_key=ck, store_key=f"perturbation_flow_{basis}", verbose=False)
    return pert
...
def _perturb_section(adata, name, ck, report, cfg, A, B, An, Bn, tag):
    ...
    if vek and vek in adata.obsm:
        # reference (WT) developmental flow = the dataset's OWN precomputed embedded velocity
        # (e.g. dynamo's velocity_umap). Used as v_ref in the inner product Delta_x . v_ref and
        # in the pairwise-KO screen -- do NOT reproject the model's WT velocity for it.
        adata.obsm[wt_key] = np.asarray(adata.obsm[vek])[:, :2].astype(float)
    else:
        # everyone else: the INPUT velocity (velocity_S) projected with the SAME correlation
        ...
        adata.obsm[wt_key] = np.asarray(_proj(vin))[:, :2].astype(float)
    ...
    report.sub(f"5.{tag}.2 Single-KO lineage bias ({An} vs {Bn})",
    ...
    # KO/OE flow-demo genes: the STRONGEST post-KO biaser in EACH lineage, taken from the
    # single-KO screen above (largest post-KO lineage bias in each direction), NOT the top
    # structural Pareto driver score.
    _sbf = (pd.Series({g: v["lineage_bias"] for g, v in single_bias.items()
                       if np.isfinite(v.get("lineage_bias", np.nan))}).sort_values()
            if single_bias else pd.Series(dtype=float))
    geneA = _sbf.index[-1] if len(_sbf) and _sbf.iloc[-1] > 0 else (A_cand[0] if A_cand else g0)
    geneB = _sbf.index[0]  if len(_sbf) and _sbf.iloc[0]  < 0 else (B_cand[0] if B_cand else None)
    _seen = set()
    flow_genes = [(g, lab) for g, lab in [(geneA, f"strongest {An}-biasing KO"),
                                          (geneB, f"strongest {Bn}-biasing KO")]
                  if g and g in adata.var_names and not (g in _seen or _seen.add(g))]
    report.sub(f"5.{tag}.3 KO vs OE flows and velocity alignment ({An} vs {Bn})",
               "The two featured genes are the strongest post-KO lineage biaser in EACH direction, "
               "taken from the single-KO screen above (not the top structural driver score). For each, "
               "the $\\Delta x$ displacement field is computed TWO ways, side by side, so they can be "
               "compared. **LEFT column** = the discrete CellOracle-style GRN signal propagation "
               "(`simulate_perturbation`, 3 propagation steps; Methods 8.1). **RIGHT column** = "
               "integrating the fitted Hopfield ODE $dx/dt = W\\sigma(x) - \\gamma x + I$ under the "
               "perturbation with the perturbed gene held fixed (`simulate_shift_ode`, 100 Euler "
               "steps; Methods 8.2). The displacement $\\Delta x$ is then PROJECTED onto the embedding "
               "with the neighbor-correlation transition-probability scheme "
               "(`calculate_flow(method='correlation')`; this is the projection, not the simulation) "
               "and drawn on a grid. Rows 2-3 are the KO/OE perturbation flows; rows 4-5 are the inner "
               "product $\\Delta x \\cdot v_{\\mathrm{ref}}$ with the wild-type flow (positive = the "
               "perturbation pushes cells along the developmental flow), grid-averaged. Row 1 is the "
               "shared context (cell types and reference velocity). " +
               (f"Here $v_{{\\mathrm{{ref}}}}$ is the dataset's own precomputed `{vek}` embedded velocity."
                if (vek and vek in adata.obsm) else
                "Here $v_{\\mathrm{ref}}$ is the input RNA velocity, projected to the embedding with the "
                "same correlation (transition-probability) scheme as the perturbation flows."))

    def _ko_oe_grid_for(gene, lab):
        try:
            gmax = float(np.percentile(np.asarray(adata[:, gene].layers["Ms"]).ravel(), 99))
            # discrete (CellOracle-style propagation) and ODE (integrated Hopfield dynamics)
            ko_d = _perturb(adata, {gene: 0.0}, ck, basis)
            oe_d = _perturb(adata, {gene: 2 * gmax}, ck, basis)
            ko_o = _perturb_ode(adata, {gene: 0.0}, ck, basis)
            oe_o = _perturb_ode(adata, {gene: 2 * gmax}, ck, basis)
            for pert in (ko_d, oe_d, ko_o, oe_o):
            ...
            def _grid():
                ...
                emb = np.asarray(adata.obsm[f"X_{basis}"])[:, :2]
                # row 1: shared context
                ax[0, 0].scatter(emb[:, 0], emb[:, 1],
                                 c=[colors.get(str(c), "#ccc") for c in adata.obs[ck].astype(str)], s=8, alpha=0.6)
                ax[0, 0].set_title("cell types"); ax[0, 0].set_axis_off()
                # reference (WT) velocity flow, projected + drawn on a grid (same projection engine)
                _flow_grid(ax[0, 1], adata, wt_key, basis, ck, colors)
                ax[0, 1].set_title(f"reference velocity (precomputed {vek})" if (vek and vek in adata.obsm)
                                   else "reference velocity (input, correlation-projected)")
                # rows 2-3: KO / OE perturbation displacement flow, correlation-projected on a grid
                for a_, pert_, ttl_ in [
                    (ax[1, 0], ko_d, f"{gene} KO $\\Delta x$  |  DISCRETE propagation"),
                    (ax[1, 1], ko_o, f"{gene} KO $\\Delta x$  |  ODE integration"),
                    (ax[2, 0], oe_d, f"{gene} OE $\\Delta x$  |  DISCRETE propagation"),
                    (ax[2, 1], oe_o, f"{gene} OE $\\Delta x$  |  ODE integration"),
                ]:
                    _flow_grid(a_, pert_, f"perturbation_flow_{basis}", basis, ck, colors)
                    a_.set_title(ttl_)
                # rows 4-5: inner product with reference velocity, discrete (left) vs ODE (right)
                ip_panels = [
                    (ax[3, 0], ko_d, f"{gene} KO: $\\Delta x \\cdot v_{{ref}}$  |  DISCRETE"),
                    (ax[3, 1], ko_o, f"{gene} KO: $\\Delta x \\cdot v_{{ref}}$  |  ODE"),
                    (ax[4, 0], oe_d, f"{gene} OE: $\\Delta x \\cdot v_{{ref}}$  |  DISCRETE"),
                    (ax[4, 1], oe_o, f"{gene} OE: $\\Delta x \\cdot v_{{ref}}$  |  ODE"),
                ]
                for a2, pert, ttl in ip_panels:
                    try:
                        sch.pl.plot_inner_product(pert, basis=basis, ax=a2, inner_product_key="ip",
                                                  on_grid=True, n_grid=40, min_mass=25)
                    except TypeError:
                        sch.pl.plot_inner_product(pert, basis=basis, ax=a2, inner_product_key="ip")
                    a2.set_title(ttl); a2.set_axis_off()
                fig.tight_layout()
                return fig
            safe = "".join(c if (c.isalnum() or c in "-.") else "_" for c in str(gene))
            _try(report, name, f"E{tag}_3_{safe}_ko_oe_grid.png", _grid,
                 f"**{gene}** ({lab}): KO/OE perturbation flows and velocity alignment, computed two "
                 f"ways. LEFT = discrete GRN signal propagation (CellOracle-style); RIGHT = ODE "
                 f"integration of the fitted Hopfield dynamics.")
        except Exception as exc:
            import traceback; traceback.print_exc()
            print(f"  [E{tag}.3 {gene}] {exc}", flush=True)

    for _g, _lab in flow_genes:
        _ko_oe_grid_for(_g, _lab)

    # ---- per-gene KO flow grid, colored by group (nb06 style) ----
    report.sub(f"5.{tag}.4 KO flows for top candidates ({An} vs {Bn})",
               "KO-induced flow (ODE integration of the fitted dynamics) for the top candidates "
               "of each lineage, colored by group "
    ...
    def _grid_flows():
        ...
        for k, (g, col) in enumerate(genes_grp):
            ...
            p = sch.dyn.simulate_shift_ode(adata, {g: 0.0}, cluster_key=ck, n_steps=100, method="euler")
            sch.tl.calculate_flow(p, source="delta", basis=basis, method="correlation",
                                  cluster_key=ck, store_key=f"perturbation_flow_{basis}", verbose=False)
            _flow_grid(axx, p, f"perturbation_flow_{basis}", basis, ck, colors)
            axx.set_title(f"{g} KO $\\Delta x$ (ODE, correlation projection)")
    ...
    report.sub(f"5.{tag}.5 Pairwise KO and epistasis ({An} vs {Bn})",
    ...
    try:
        ...
        def _combined():
            ...
            # dashed staircase along the LOWER edge of the diagonal, so the diagonal single-KO
            # cells belong to the upper/bias triangle (previously drawn on the upper edge, one
            # unit too high).
            xs, ys = [], []
            for i in range(n):
    ...
    # ---- energy change ----
    report.sub(f"5.{tag}.6 Energy change under KO ({An} vs {Bn})",
               f"Per-cell-type change in total energy when {g0} is knocked out (ODE integration).")
    def _de():
        order2 = order_by_trajectory(adata, ck, present_clusters(adata, ck), cfg.get("lineages"), explicit=cfg.get("order"))
        k = sch.dyn.simulate_shift_ode(adata, {g0: 0.0}, cluster_key=ck, n_steps=100, method="euler")
    ...
    report.sub(f"5.{tag}.7 Dose-response ({An} vs {Bn})",
    ...
    report.sub(f"5.{tag}.8 Anchor-partner double-KO recipes ({An} vs {Bn})",
    ...
    def _recipe():
        fig, axes = plt.subplots(1, len(anchors),
                                 figsize=(6.4 * len(anchors), max(4.8, 0.38 * N_PARTNERS)), squeeze=False)
        for j, anchor in enumerate(anchors):
            pw = sch.tl.grn_partner_weights(adata, anchor)
            partners = [g for g in pw.abs().mean(axis=1).sort_values(ascending=False).index if g != anchor][:N_PARTNERS]
            ...
            # anchor's own single-KO bias: partners whose double-KO lands on the OTHER side of
            # this line have flipped the anchor's effect (the sign-flippers the recipe surfaces).
            if isinstance(single_bias, dict) and anchor in single_bias:
                ax.axvline(single_bias[anchor]["lineage_bias"], color="grey", ls="--", lw=0.9)
            ax.set(xlabel=f"double-KO lineage bias (+{An} / -{Bn})",
                   title=f"anchor {anchor} (grey dashed = {anchor} single-KO)")
        fig.tight_layout()
        return fig
    if anchors:
        _try(report, name, f"E{tag}_8_anchor_partner.png", _recipe,
             f"Anchor-partner double KOs for anchors {anchors} ({N_PARTNERS} partners each; "
             f"partners crossing the grey dashed line flip the anchor's single-KO direction).")


def _fate_perturbation_subsection(adata, name, ck, report, cfg, tag):
    """Fate-probability perturbation effect (+ reprogramming), rendered from the cached fate metric
    (reports/<name>/data/perturb_dynamics.pkl, written by reproducibility/compute/_perturb_dynamics_compute.py).
    Projection-free knockout readout: each cell's change in the terminal-fate split fraction
    (fate_KO - fate_WT) under the fitted field made absorbing at the lineage attractors. Committed
    cells that flip fate (black rings) are transdifferentiation; the bulk of the effect is the
    redirection of the still-deciding cells at the fate boundary. Read from the cache so the report
    matches the paper figure exactly and needs no recompute; skipped if the cache is absent."""
    import pickle
    cache_p = f"{ROOT_}/{name}/data/perturb_dynamics.pkl"
    if not os.path.exists(cache_p):
        report.text("> _(fate-probability perturbation effect skipped: run "
                    "`reproducibility/compute/_perturb_dynamics_compute.py --dataset " + name + "` first.)_")
        return
    with open(cache_p, "rb") as fh:
        C = pickle.load(fh)
    fmap = C.get("fate_map") or {}
    if not any(isinstance(v, dict) and "shift" in v for v in fmap.values()):
        return                                                  # older cache without the per-cell shift
    emb = np.asarray(C["emb"]); tfs = list(C["tfs"]); groups = C.get("tf_groups", {})
    lps = C["lineage_pairs"]; fate_bias = C.get("fate_bias", {}); fate_pvals = C.get("fate_pvals", {})
    gnames = list(groups)
    gpi = {g: k for k, gn in enumerate(gnames) for g in groups[gn]}   # gene -> lineage-pair index
    COMMIT, REPROG = 0.80, 0.05

    report.sub(
        f"5.{tag} Fate-probability perturbation effect (reprogramming)",
        "Projection-free knockout readout (Methods, fate-probability lineage effect): each cell is "
        "colored by its change in the terminal-fate split fraction after the KO, on that gene's "
        "lineage decision. Gray = unchanged (committed and stable); color = the cells the KO moves "
        "(+ toward the first lineage / - toward the second). **Black rings** mark already-committed "
        "cells the KO flips to the other fate (genuine transdifferentiation). Mature-cell "
        "dedifferentiation is structurally near-absent here because the model integrates each cell "
        "under its own cell-type GRN.")

    def draw():
        n = len(tfs)
        fig = plt.figure(figsize=(2.35 * n + 0.8, 3.4))
        gs = fig.add_gridspec(1, n, left=0.02, right=0.98, top=0.84, bottom=0.24, wspace=0.05)
        order = sorted(range(n), key=lambda j: gpi.get(tfs[j], 0))
        col = 0; sc_by_pair = {}
        for j in order:
            g = tfs[j]; k = gpi.get(g, 0); _, _, An, Bn = lps[k]
            entry = fmap.get((An, Bn), {})
            shift = np.asarray(entry.get("shift", {}).get(g)) if entry else None
            s0 = np.asarray(entry.get("split_wt")) if entry else None
            ax = fig.add_subplot(gs[0, col]); col += 1
            ax.set_title(f"{g} KO", fontsize=9); ax.set_axis_off()
            if shift is None:
                continue
            grp = [np.abs(np.asarray(fmap[(An, Bn)]["shift"][gg]))
                   for gg in groups[gnames[k]] if gg in fmap.get((An, Bn), {}).get("shift", {})]
            vmax = float(np.percentile(np.concatenate(grp), 99)) if grp else 1e-6
            vmax = vmax or 1e-6
            eps = max(0.06 * vmax, 1e-4)
            ax.scatter(emb[:, 0], emb[:, 1], c="0.85", s=3, lw=0, zorder=0)
            m = np.abs(shift) >= eps
            idx = np.where(m)[0][np.argsort(np.abs(shift[m]))]
            s = ax.scatter(emb[idx, 0], emb[idx, 1], c=shift[idx], cmap="RdBu_r",
                           vmin=-vmax, vmax=vmax, s=6, lw=0, zorder=1)
            c0 = np.maximum(s0, 1.0 - s0)
            reprog = (c0 > COMMIT) & (np.abs(shift) > REPROG) & (np.sign(shift) == np.sign(0.5 - s0))
            if reprog.any():
                ax.scatter(emb[reprog, 0], emb[reprog, 1], s=22, facecolors="none",
                           edgecolors="k", linewidths=0.6, zorder=2)
            sc_by_pair[(An, Bn)] = (s, vmax)
        pairs_seen = list(dict.fromkeys((lps[gpi.get(g, 0)][2], lps[gpi.get(g, 0)][3]) for g in tfs))
        for i, (An, Bn) in enumerate(pairs_seen):
            if (An, Bn) not in sc_by_pair:
                continue
            s, vmax = sc_by_pair[(An, Bn)]
            cx = (i + 0.5) / max(len(pairs_seen), 1)
            cax = fig.add_axes([cx - 0.11, 0.09, 0.22, 0.03])
            ...
            cax.text(-0.03, 0.5, Bn, transform=cax.transAxes, ha="right", va="center", fontsize=7)
            cax.text(1.03, 0.5, An, transform=cax.transAxes, ha="left", va="center", fontsize=7)
            cax.set_title(f"post-KO fate shift  (±{vmax:.2g})", fontsize=6)
        return fig

    _try(report, name, "E_fate_shift_maps.png", draw,
         "Per-cell fate-probability shift after single-gene knockout, grouped by lineage decision. "
         "Gray = committed/unchanged; color = the deciding cells the KO redirects; black rings = "
         "already-committed cells that reprogram to the other fate.")

    md = ["", "| gene | decision | decider mean shift | Wilcoxon p | max |shift| | committed | reprogrammed |",
          "|---|---|---:|---:|---:|---:|---:|"]
    for g in tfs:
        k = gpi.get(g, 0); _, _, An, Bn = lps[k]
        entry = fmap.get((An, Bn), {})
        shift = np.asarray(entry.get("shift", {}).get(g)) if entry else None
        s0 = np.asarray(entry.get("split_wt")) if entry else None
        if shift is None:
            continue
        c0 = np.maximum(s0, 1.0 - s0); committed = c0 > COMMIT
        reprog = (c0 > COMMIT) & (np.abs(shift) > REPROG) & (np.sign(shift) == np.sign(0.5 - s0))
        ms = fate_bias.get((An, Bn), pd.Series(dtype=float)).get(g, np.nan)
        p = fate_pvals.get((An, Bn), pd.Series(dtype=float)).get(g, np.nan)
        md.append(f"| {g} | {An} vs {Bn} | {ms:+.4f} | {p:.1e} | {float(np.abs(shift).max()):.3f} "
                  f"| {int(committed.sum())} | {int(reprog.sum())} |")
    report.text("\n".join(md))
    report.text(
        "*Decider mean shift* = mean split-fraction change over the transitional decider cells "
        "(the panel-e/f number); *committed* = cells whose WT commitment to a fate exceeds 0.80; "
        "*reprogrammed* = committed cells the KO flips by more than 0.05 toward the other lineage. "
        "Small reprogrammed counts are expected: the fitted attractors are robust, so single-gene "
        "transdifferentiation is marginal and mature-cell dedifferentiation is structurally absent.")
...
def section_E(adata, name, ck, report, cfg):
    ...
    _fate_perturbation_subsection(adata, name, ck, report, cfg, tag=len(pairs) + 1)
