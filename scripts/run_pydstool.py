...
# Monkey-patch legacy numpy types required by PyDSTool
np.int = int
np.float = float
np.bool = bool
np.complex = complex
...
# Parameters
I1 = 3.9961299896240234
I2 = 3.997412919998169
gamma1 = 2.995926856994629
gamma2 = 2.9975945949554443
w11 = 4.998366832733154
w22 = 4.999235153198242
...
# Initial condition near the neutral equilibrium
...
PC.newCurve(PCargs)
print("Computing Central Branch...")
PC['EQ1'].forward()
PC['EQ1'].backward()
...
print("Computing Upper Stable Branch...")
PC['EQ_up'].forward()
...
print("Computing Lower Stable Branch...")
PC['EQ_down'].forward()

try:
    plt.figure(figsize=(10,6))
    # PC.display automatically overlays all successfully calculated branches!
    PC.display(['c', 'x1'], stability=True, figure=1)

    plt.title('Pitchfork Bifurcation of the Toggle Switch (PyDSTool PyCont)', fontsize=14)
    plt.xlabel('Mutual Inhibition (c)', fontsize=12)
    plt.ylabel('Gene 1 Steady State', fontsize=12)
    plt.xlim(-15.5, 1.0)
    plt.grid(True, alpha=0.3)

    plt.savefig('pydstool_bifurcation.png', dpi=300, bbox_inches='tight')
    print("SUCCESS: Full Pitchfork Image generated!")
