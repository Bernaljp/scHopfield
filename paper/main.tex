...
\subsection*{Regulatory and dynamical conclusions are robust to the choice of velocity estimator}
Because scHopfield regresses the inferred dynamics against an estimated RNA velocity, we asked how much its downstream outputs depend on which velocity estimator is used upstream, rather than on how similar the velocity estimates themselves are. We refit pancreatic endocrinogenesis, murine neural crest, and a dynamo-processed hematopoiesis dataset against velocities obtained from scVelo, from dynamo, and from a pseudotime-derived estimator, holding the cells, the gene set, the activation functions, and the scaffold fixed so that only the velocity target changed. The dynamical readouts we interpret were preserved across estimators: for every estimator the fitted velocity field projected to the same progenitor-to-terminal differentiation streams (Supplementary Figure S7), so no estimator produced a degenerate or reversed landscape. The nominated lineage drivers were concordant where the estimators broadly agreed (top-driver Jaccard up to 0.90 on neural crest) and less concordant on the hematopoiesis data, where the estimators disagreed most. Across datasets the sensitivity to the estimator fell on the individual interaction weights rather than on the projected flow or the well-determined driver nominations, consistent with our practice of interpreting driver-level and landscape-level conclusions rather than individual edge weights. Finally, because the pseudotime estimator requires only a cell ordering, scHopfield applies directly to datasets and protocols that lack spliced and unspliced counts, extending it beyond splicing-based velocity.
...
The closed-form estimate is $O(m \log m)$ (dominated by sorting) and requires no iterative optimisation.

\textbf{Bounded nonlinear refinement.} The closed-form fit can be unstable for genes with few active cells or heavy-tailed ECDFs (it can return $n \le 0$ or an exploding $k$). The estimate is therefore constrained to $n \in [1, n_{\max}]$ and $0 < k \le \max_j x_j$ (with $n_{\max}=20$; a ceiling of $8$ was found to truncate sharply switching genes) and refined by a bounded nonlinear least-squares fit of $\varphi$ to the ECDF. The refinement is run from two initialisations, the closed-form $(k,n)$ and $(2k, 2n)$, and the fit with the lower MSE is retained; genes with two expression regimes have a poor optimum near the closed-form initialisation, and the second start recovers the steeper/higher-threshold regime. Degenerate genes (all-zero, constant, or fewer than five active cells) are handled explicitly rather than returning invalid parameters.

\textbf{Two-component (bimodal) Hill.} Some genes have a genuinely bimodal expression CDF (two sub-populations with distinct activation thresholds) that a single Hill cannot represent. For these, a two-component mixture is fit,

\begin{equation}
\varphi_{\text{bi}}(x) = a\,\varphi(x; k_1, n_1) + (1-a)\,\varphi(x; k_2, n_2), 
\end{equation}

but only when the expression distribution is actually bimodal (Sarle's bimodality coefficient $> 0.6$) and the mixture both improves the MSE by at least $40\%$ and yields well-separated thresholds ($\max(k_1,k_2)/\min(k_1,k_2) \ge 2$) with a minor-component weight $\min(a, 1-a) \ge 0.1$; otherwise the single Hill is kept. When accepted, the second component $(k_2, n_2)$ and the weight $a$ are stored, each cell is assigned to its nearest Hill regime (component 1 if $|x-k_1| < |x-k_2|$, else component 2), and that regime's activation is used both for the sigmoid layer and for the inverse-sigmoid integral of the degradation energy (§4). Single-Hill genes ($a = 1$) are unaffected, so the model reduces to the single-Hill case when no bimodality is detected.
...
\textbf{Network parametrisation.} The interaction matrix $W$ is a learnable $N \times N$ weight matrix; the bias vector $I \in \mathbb{R}^N$ is learnable. The degradation rates $\gamma_i$ are log-parametrised as $\gamma_i = \exp(\tilde{\gamma}_i)$ to enforce positivity and, when trained, are capped at a biologically plausible maximum $\gamma_i = \min(\exp(\tilde{\gamma}_i), \gamma_{\max})$ (default $\gamma_{\max}=10$) to prevent a runaway rate; by default they are pre-estimated and held fixed.
...
- \textbf{Bias regularisation}: by default $\mathcal{L}_{\text{bias}} = \|I\|_1$ (lasso). Because the activation $\varphi(X)$ is nearly constant within a cell type, the bias term $I$ is confounded with $W\varphi(X)$ and, if unpenalised or penalised in $L^2$, absorbs most of the signal (the "bias takeover": on the datasets studied, an unpenalised or no-scaffold fit places $40$--$95\%$ of the total energy in the bias term). The $L^1$ penalty is sparse, driving most genes to exactly zero bias while allowing a few genuinely externally forced genes to grow, which matches the expectation that a natural differentiation has small bias. An $L^2$ (group-shrinkage) and an elastic-net variant are also available. With the scaffold prior plus the $L^1$ bias the bias energy collapses to $\approx 0\%$ across datasets.
...
where the constant $C_0 = \frac{-n\,k}{n-1}\;{}_2F_1\!\left(-\frac{1}{n},\frac{n-1}{n};\frac{2n-1}{n};\,1\right)$ ensures the integral vanishes at $\sigma = 0$. For genes fitted with the two-component Hill (§2), the degradation integral is evaluated with the $(k, n)$ of the component assigned to each cell by its regime label $r_i$ (the nearer of the two thresholds), so the energy is consistent with the activation used in the dynamics.
...
\textbf{Streamline visualisation of the model velocity field.} For publication-quality figures, the fitted Hopfield velocity $\dot{\mathbf{x}} = W\boldsymbol{\sigma}(\mathbf{x}) + \mathbf{I} - \gamma\mathbf{x}$ (or the wild-type/perturbed difference field) is written as a velocity layer and projected onto the embedding with the established transition-probability streamline estimators of scVelo (`velocity\_embedding\_stream`) or Dynamo (`streamline\_plot`), rather than the per-cell KNN average of eq. (35). These estimators pool the local transition kernel over the neighbourhood graph and yield smooth, coherent streamlines from progenitors to terminal states; the ad-hoc KNN projection produced visually chaotic arrows even when the underlying gene-space reconstruction cosine was high ($\approx 0.81$), so it is used only for quick diagnostics.
...
This flow-alignment metric is exposed as `compute\_perturbation\_flow\_bias` (formerly `compute\_lineage\_bias`), renamed to disambiguate it from the structural driver score `score\_driver\_tfs` (§8.4), which returns identically named fields but a different quantity. `score\_driver\_tfs` combines three per-lineage signals (mean $W$-row norm, out-degree centrality, and energy--gene correlation); these are standardised (z-scored across genes) before averaging so they contribute on a comparable scale, rather than being combined as a raw rank sum.

\textbf{Pre/post lineage-commitment change (`compute\_perturbation\_commitment\_change`).} The flow-alignment score above is relative to the wild-type flow but is not a matched before/after difference. A cleaner, model-independent pre/post score is defined from a data-derived lineage axis $\mathbf{a} = (\bar{p}_A - \bar{p}_B)/\|\bar{p}_A - \bar{p}_B\|$, where $\bar{p}_A, \bar{p}_B$ are the embedding centroids of lineages $A$ and $B$. The commitment of a cell under a flow field $\mathbf{f}$ is the cosine alignment $c = \hat{\mathbf{f}} \cdot \mathbf{a}$. Evaluating this for the wild-type flow (pre) and the perturbed flow (post) gives $\Delta c_i = c_i^{\text{post}} - c_i^{\text{pre}}$; a positive value means the perturbation steers cells toward lineage $A$, negative toward $B$. Because both terms are cosines against the same fixed, model-independent axis, the pre/post comparison is normalised and directly interpretable.
...
where $p_{ij}$ are the row-normalised forward-graph weights, $\mathbf{x}_j - \mathbf{x}_i$ is the expression difference between cells $j$ and $i$, and $t_j - t_i$ is the corresponding pseudotime interval. Cells without forward pseudotime neighbours receive zero velocity.

\textbf{Robustification.} A fixed floor $\varepsilon = 10^{-6}$ on the denominator is far too small relative to the pseudotime scale: cells whose forward neighbours have nearly equal pseudotime ($\Delta t \to 0$) acquire exploding velocities (on the hematopoiesis pseudotime data, the largest per-cell speed exceeded the median by $\sim\!430\times$), which corrupt both the GRN fit and the projected velocity field. The denominator is therefore floored relative to the local pseudotime scale, $\Delta \bar t_i \leftarrow \operatorname{sign}(\Delta \bar t_i)\,\max(|\Delta \bar t_i|,\, 10^{-2}\,\operatorname{median}|\Delta \bar t|)$, and the per-cell velocity magnitude is winsorised at the $99$th percentile (direction preserved). This reduces the outlier ratio to $\sim\!9\times$, comparable to splicing-based velocity.
...
For datasets that lack a splicing-based velocity (for example the Paul et al. hematopoiesis data, which is provided as a diffusion-pseudotime trajectory), scHopfield derives the velocity target from pseudotime with the robustified estimator of Section 9, so that the GRN fit and all downstream analyses apply unchanged. Where a dataset provides more than one velocity source, the same fitting procedure applies to each; the choice of source (splicing-based scVelo/Dynamo velocity versus pseudotime-derived velocity) is treated as a preprocessing decision and its effect on the fitted matrices and driver rankings is assessed empirically rather than assumed.
